...
def run_recognition_tab():
    ...
    st.markdown("""
        <div class="camera-section">
            <div class="camera-header-title">📷 스마트 AI 품목 분석기</div>
            <p style="font-size: 0.9rem; color: #4A5A4E; margin-bottom: 0rem; line-height: 1.5;">
                HOG(형태 분석) 엔진이 적용된 고정밀 분석기입니다. 사진을 촬영하거나 업로드하여 올바른 배출 방법을 확인하세요.
            </p>
        </div>
    """, unsafe_allow_html=True)

    model, labels = load_ai_model_v2()
    ...
    if active_image:
        if model is None:
            st.info("💡 모델 학습 파일을 찾을 수 없습니다. 학습 완료 후 다시 시도해 주세요.")
            st.image(active_image, caption="업로드된 이미지", width='stretch')
        else:
            # 이미지 로드
            img = Image.open(active_image).convert("RGB")
            st.image(img, caption="분석 대상 이미지", width='stretch')
            
            with st.spinner("이미지의 형태를 정밀 분석 중입니다..."):
                # 학습 시와 동일한 HOG 특징 추출 과정
                img_resized = img.resize((128, 128))
                img_np = np.array(img_resized)
                gray_img = rgb2gray(img_np)
                
                # HOG 특징 추출
                feat = hog(gray_img, orientations=9, pixels_per_cell=(16, 16),
                          cells_per_block=(2, 2), visualize=False)
                feat = feat.reshape(1, -1)
                
                # 예측
                class_idx = model.predict(feat)[0]
            ...
            guide_text = guides.get(result_label, "해당 품목의 상세 배출 가이드는 '분리배출 가이드' 탭을 참고하세요.")
